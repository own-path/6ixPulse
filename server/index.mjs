...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/agent/run") {
      ...
      const localRun = buildLocalAgentRun(prompt);
        ...
        modelRun.status === "done" && modelRun.result
          ? {
              ...mergeModelRecommendation(
                ...
                modelRun.result,
                modelRun.model,
                modelRun.provider === "ollama" ? "ollama" : modelRun.provider === "hf" ? "hf" : "nvidia",
              ),
              provider: modelRun.provider,
            }
          : {
...
async function runConfiguredModel(localRun) {
  ...
  return {
    provider: "auto",
    status: "error",
    reason: `NVIDIA: ${nvidia.reason}; Ollama: ${ollama.reason}; Hugging Face: ${hf.reason}`,
    model: configuredAgentModel(),
    result: null,
  };
}
...
function configuredProvider() {
  ...
  if (raw === "nvidia" || raw === "ollama" || raw === "hf" || raw === "auto") return raw;
  return "hf";
}
...
function configuredAgentModel() {
  ...
  if (provider === "auto") return configuredNvidiaModel() || configuredOllamaModel() || configuredModel();
